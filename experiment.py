...
tf.flags.DEFINE_integer('eval_size', 26032, 'Size of the test dataset.')
  ...
  for i in range(num_gpus):
    with tf.device('/gpu:%d' % i):
      if dataset == 'mnist':
        ...
        raise ValueError(
            'Unexpected dataset {!r}, must be mnist, norb, cifar10 or svhn.'.format(
                dataset))
  return features
...
def default_hparams():
  ...
  return tf.contrib.training.HParams(
      ...
      # learning_rate=0.0005,  # test4 -- 21.94
      loss_type='margin',
